...
<form action="{{ route('study-records.store') }}" method="POST">
    ...
    <div class="w-1/2 mx-auto flex gap-5 justify-center mb-5">
        <labe class="w-1/4 bg-blue-300 py-2 px-4 rounded-lg text-center">カテゴリ</labe>
        <input class="w-3/4 border pl-2" type="text" name="category" value="{{ old('category') }}">
        @error('category')
        <p>{{ $message }}</p>
        @enderror
    </div>
    ...
    <button class="bg-green-400 py-2 px-4 rounded-lg text-center mx-auto justify-center block" type="submit">登録する</button>
